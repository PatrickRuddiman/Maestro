...
// Pre-compiled static regex patterns (avoid re-creation on every render/call)
...
const AutoRunInner = forwardRef<AutoRunHandle, AutoRunProps>(function AutoRunInner(
	...
	// Memoize search regex - only recompute when query changes
	// Used by both the debounced match counter and scroll-to-match effect
	const searchRegex = useMemo(() => {
		const trimmed = searchQuery.trim();
		if (!trimmed) return null;
		const escapedQuery = trimmed.replace(SEARCH_ESCAPE_REGEX, '\\$&');
	...
	useEffect(() => {
		...
		if (!searchOpen || !searchRegex || totalMatches === 0) return;
	...
	}, [currentMatchIndex, searchOpen, searchRegex, searchQuery, totalMatches, mode, localContent]);
