...
describe('parsers/index', () => {
	...
	describe('initializeOutputParsers', () => {
		...
		it('should register Cline parser', () => {
			expect(hasOutputParser('cline')).toBe(false);

			initializeOutputParsers();

			expect(hasOutputParser('cline')).toBe(true);
		});
		...
		it('should register exactly 5 parsers', () => {
			...
			expect(parsers.length).toBe(5); // Claude, OpenCode, Codex, Factory Droid, Cline
		});
	...
	describe('getOutputParser', () => {
		...
		it('should return ClineOutputParser for cline', () => {
			const parser = getOutputParser('cline');
			expect(parser).not.toBeNull();
			expect(parser).toBeInstanceOf(ClineOutputParser);
		});
	...
	describe('parser exports', () => {
		...
		it('should export ClineOutputParser class', () => {
	...
	describe('integration', () => {
		...
		it('should correctly parse Cline output after initialization', () => {
			initializeOutputParsers();

			const parser = getOutputParser('cline');
			// Cline uses task_started for session initialization with taskId
			const event = parser?.parseJsonLine(
				JSON.stringify({ type: 'task_started', taskId: 'cline-789' })
			);

			expect(event?.type).toBe('init');
			expect(event?.sessionId).toBe('cline-789');
		});
	});
});
